      ...
      <div className="grid grid-cols-3 gap-2">
        {slots.map((item, idx) => (
          <button
            ...
            disabled={!item}
            aria-label={item ? `${item.name}, ${item.category || 'item'}, quantity ${item.quantity}` : `Empty inventory slot ${idx + 1}`}
          ...
          >
            {item ? (
              <>
                ...
                {item.category && (
                  <span className="absolute left-1 top-1 rounded bg-slate-950/75 px-1 text-[7px] font-bold uppercase tracking-wide text-cyan-200">
                    {item.category}
                  </span>
                )}
                {item.quantity > 1 && (
                ...
                <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 w-32 bg-slate-900/95 border border-white/10 p-2 rounded shadow-xl opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none z-50">
                  ...
                  {item.consumable === false && <p className="mt-0.5 text-[8px] font-bold uppercase tracking-wide text-cyan-200">Reusable</p>}
